...
export default function AuditProcessListPage() {
  ...
  useEffect(() => {
    ...
    const fetchProcesses = async () => {
      try {
        let data: AuditProcess[] = [];
        if (user.role !== "ExternalAuditor") {
          const res = await apiClient.get<{ data: AuditProcess[] }>(`/audits/api/auditProcesses/getAll?businessId=${businessId}&rulesetId=${rulesetId}`);
          data = res.data || [];
        } else {
          const res = await apiClient.get<{ data: AuditProcess[] }>(`/audits/api/`);
          // Filtrar: solo procesos NOT_EVALUATED donde el usuario es auditor externo
          data = (res.data || []).filter(proc => {
  ...
  return (
    <div className="max-w-2xl mx-auto p-6">
      <h1 className="text-3xl font-extrabold mb-8 text-center text-primary-color tracking-tight drop-shadow-sm">
        {t("audits.processList.title", "Audit Processes")}
      </h1>
      {processes.length === 0 ? (
        <div className="flex flex-col items-center justify-center text-gray-500 py-16">
          ...
          <span className="text-lg font-medium">{t("audits.processList.empty", "No Non Evaluated Audit Processes found.")}</span>
        </div>
      ) : (
            ...
            >
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="font-bold text-lg text-primary-color group-hover:underline tracking-tight">{idx + 1}</div>
                ...
                <div className="flex flex-col items-end gap-1 min-w-[120px]">
                  ...
                  <div className="text-xs text-gray-400 mt-1">
                    {proc.assesments?.length ?? 0} {t("audits.processList.assessments", "assessments")}
                  </div>
                </div>
              </div>
            </Card>
